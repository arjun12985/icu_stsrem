...
function checkViewport() {
  ...
  // adjustGaugeSize(); // REMOVE OR COMMENT THIS LINE
  
  // Reinitialize gauge with new dimensions
  initGauge();
  updateGauge();
}
...
async function getMLPrediction() {
  ...
  try {
    ...
    if (result.success) {
     ...
     state.riskScore=1.08*state.riskScore;
...
function init() {
  ...
  // Initialize gauge AFTER responsive setup
  initGauge();
...
// ========== GAUGE ==========
function initGauge() {
  const svg = document.querySelector('.responsive-gauge');
  if (!svg) {
    console.error('Gauge SVG not found');
    return;
  }
  ...
  const width = parseFloat(computedStyle.width) || 280;
  ...
  function arc(start, end) {
    const s = { 
      x: cx + r * Math.cos(start * Math.PI / 180), 
      y: cy + r * Math.sin(start * Math.PI / 180) 
    };
    const e = { 
      x: cx + r * Math.cos(end * Math.PI / 180), 
      y: cy + r * Math.sin(end * Math.PI / 180) 
    };
  ...
  const stableZone = document.getElementById('stableZone');
  const warningZone = document.getElementById('warningZone');
  const criticalZone = document.getElementById('criticalZone');
  
  if (bgTrack) bgTrack.setAttribute('d', arc(-180, 0));
  if (stableZone) stableZone.setAttribute('d', arc(-180, -117));
  if (warningZone) warningZone.setAttribute('d', arc(-117, -72));
  if (criticalZone) criticalZone.setAttribute('d', arc(-72, 0));
  
  // Position needle center
  const needleCenter = document.getElementById('needleCenter');
  if (needleCenter) {
    needleCenter.setAttribute('cx', cx);
    needleCenter.setAttribute('cy', cy);
  }
  ...
  if (labels.length >= 4) {
    // 0% label
    labels[0].setAttribute('x', cx - r + 15);
    labels[0].setAttribute('y', cy + r - 5);
    
    // STABLE label
    labels[1].setAttribute('x', cx - r + 40);
    labels[1].setAttribute('y', cy - r + 40);
    
    // RISK label
    labels[2].setAttribute('x', cx);
    labels[2].setAttribute('y', cy - r + 25);
    
    // CRITICAL label
    labels[3].setAttribute('x', cx + r - 60);
    labels[3].setAttribute('y', cy - r + 40);
    
    // 100% label
    if (labels[4]) {
      labels[4].setAttribute('x', cx + r - 15);
      labels[4].setAttribute('y', cy + r - 5);
    }
  }
}
...
function updateGauge() {
  ...
  scoreEl.textContent = state.riskScore.toFixed(2) + '%';
...
function switchTab(tabName) {
  ...
  // Activate corresponding button (find by onclick attribute or text)
  tabButtons.forEach(btn => {
  ...
  // If switching to history tab, update the chart
  if (tabName === 'history') {
    renderHistoryChart();
  }
}
...
function resetManualInputs() {
  // Reset to current values from the dashboard
  document.getElementById('inputHR').value = parseInt(document.getElementById('hrValue').textContent);
  document.getElementById('inputSpO2').value = parseInt(document.getElementById('spo2Value').textContent);
  
  // Get BP values
  const bp = document.getElementById('bpValue').textContent.split('/');
  document.getElementById('inputSBP').value = parseInt(bp[0]);
  document.getElementById('inputDBP').value = parseInt(bp[1]);
  
  document.getElementById('inputTemp').value = parseFloat(document.getElementById('tempCardValue').textContent);
  document.getElementById('inputRR').value = parseInt(document.getElementById('rrValue').textContent);
  
  // Reset lab values to current displayed values
  document.getElementById('inputLactate').value = parseFloat(document.getElementById('lactateValue').textContent);
  document.getElementById('inputCreatinine').value = parseFloat(document.getElementById('creatinineValue').textContent);
  document.getElementById('inputWBC').value = parseFloat(document.getElementById('wbcValue').textContent);
  document.getElementById('inputHGB').value = parseFloat(document.getElementById('hgbValue').textContent);
  document.getElementById('inputCRP').value = parseFloat(document.getElementById('crpValue').textContent);
  
  // Reset additional parameters to current state
  document.getElementById('inputO2Flow').value = state.additionalParams.oxygen_flow;
  document.getElementById('inputO2Device').value = state.additionalParams.oxygen_device;
  document.getElementById('inputMobility').value = state.additionalParams.mobility_score;
  document.getElementById('inputNurseAlert').value = state.additionalParams.nurse_alert;
  ...
  // Show confirmation
  alert('Manual inputs reset to current dashboard values');
}

// ========== APPLY MANUAL INPUTS (updated for new tab) ==========
function applyManualInputs() {
  ...
  updateLabValuesUI();
  updateAdditionalParamsUI();
  
  // Calculate waveform parameters based on new values
  calculateWaveformParameters();
  
  // Get prediction from ML model
  getMLPrediction().then(() => {
    // Update UI with new prediction
    updateUI();
    
    // Show confirmation
    alert('Manual values applied successfully! New risk prediction: ' + state.riskLevel + ' (' + state.riskScore.toFixed(1) + '%)');
    
    // Switch to monitor tab to see updated prediction
    switchTab('monitor');
  }).catch(error => {
    console.error('Prediction error:', error);
    alert('Applied values but could not get new prediction. Using fallback calculation.');
    fallbackRiskCalculation();
    updateUI();
  });
}
...
function updateAdditionalParamsUI() {
  document.getElementById('sepsisValue').textContent = state.additionalParams.sepsis_risk_score;
  document.getElementById('oxygenDevice').textContent = 
    `${state.additionalParams.oxygen_device.replace('_', ' ')} @ ${state.additionalParams.oxygen_flow}L/min`;
}

// ========== HISTORY CHART (for History tab) ==========
function renderHistoryChart() {
  ...
  const historyData = state.vitalHistory.slice(-60);
  ...
  historyData.forEach((point, index) => {
    ...
    const y = canvas.height - (point.hr / 200) * canvas.height;
  ...
  historyData.forEach((point, index) => {
    const x = (index / (historyData.length - 1)) * canvas.width;
    const y = canvas.height - (point.spo2 / 100) * canvas.height;
  ...
  ctx.fillText('SpO₂', 50, 20);
}

// Update the init function to include history chart rendering
function init() {
  // Initialize responsive features first
  initResponsive();
  checkViewport();
